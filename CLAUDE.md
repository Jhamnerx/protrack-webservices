...
- **Mapeo de evento** (`SatelTrackFormatter::resolveEvento`): los eventos de motor 500/501 son de **transición**, NO el estado continuo del ACC. El reporte periódico normal es `"2"` (Posición). Se compara el `accstatus` actual con `devices.last_accstatus`: ACC pasa a ON (`acc=1`, anterior≠1)→`"501"` (Motor Encendido); ACC pasa a OFF (`acc=0`, anterior=1)→`"500"` (Motor Apagado); sin cambio / `accstatus=-1` / sin estado previo→`"2"`. El `SatelTrackSender` persiste `last_accstatus` en cada envío exitoso (solo cuando `accstatus≠-1`). En el reenvío de historial (`ReenviarHistorialSatelTrack`) el playback no trae `accstatus`, por lo que usa `"2"` fijo.
- **`devices.last_accstatus`** (columna `tinyInteger` nullable, migración `2026_06_22_000000`): último estado conocido del ACC (1/0/null) para detectar la transición de motor.
- **Odómetro virtual**: Protrack devuelve `odometer=-1`/`mileage=-1` (no soportado), así que se construye un odómetro propio. `SatelTrackFormatter::resolveOdometro` acumula la distancia (haversine) entre `devices.last_position` (último punto enviado) y el punto actual; guarda el total en `devices.odometer_meters` (migración `2026_06_22_000001`) vía el Sender, y reporta `odometro` en km. Solo acumula si el vehículo está **en movimiento** (`speed > 0` y `accstatus != 0`, es decir motor encendido o desconocido). Guardas anti-ruido: descarta deltas `< 20 m` (jitter) y `> 50 km` (glitch). Si la API llegara a devolver odómetro real (>0) se usa ese valor. El **km inicial/actual es editable por dispositivo** desde la tabla de Config (`Config::updateOdometer`, input "Km" en Acciones, visible cuando Tracklog está activo); guarda `km * 1000` en `odometer_meters`.
- **Anti-duplicados**: la deduplicación principal es por tiempo en BD (`Processor` compara `hearttime` vs `devices.last_update`). Como el `SatelTrackSender` actualiza `last_update` de forma **asíncrona** (corre en otra cola), `ProcessUnitsJob` avanza el cursor `last_update`/`latest_position_id` **en el mismo dispatch** de `SendToSatelTrackJob`, para que el siguiente ciclo (cada 30s) no reenvíe la misma posición mientras el envío sigue encolado. Complementos: `SatelTrackSender::dedupeTramas` (elimina repetidos dentro de un lote por `placa|fechaEvento|horaEvento`) y `api:get` con `withoutOverlapping()`. **Trade-off**: si un envío falla, esa posición queda marcada como enviada (cursor ya avanzó); se recupera con el reenvío manual de historial.
